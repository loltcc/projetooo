...
if(!isset($_SESSION["system_control"]))
{
  ...
  if($system_control == 1 && $privilegio == 2){
    ...
    $sql_pesquisa ="SELECT * FROM `admin` WHERE `cod_login` = $cod_login";
    ...
    <body style="overflow-x: hidden;">
              ...
              <div class="col-8">
                <div class="col-sm" id="chamados" style="display: none;"><br>
                  ...
                  <ul style="text-align: left;" class="list-group">
                    ...
                    <?php
                    $sql_servico = "SELECT * FROM `servico` WHERE  `cod_oficina` = $cod_oficina && `status`=1 && `cod_professor` = $cod_professor";
                    ...
                    if ($numero_servico !=0) {
                      while ($vetor_servico = mysqli_fetch_array($query_servico)) {
                        ...
                        Veículo:".$vetor_veiculo['placa']."<a href='chat_professor.php?cod_servico=".$vetor_servico['cod_servico']."' style='float:right; right:0px;margin-left:5px;'><i class='fas fa-external-link-alt'></i></a>
                        Protocolo:   ".$vetor_servico['protocolo']."<br>Status: Aguardando aceitação da Oficina<br>
                        Serviço desejado:".$vetor_servico['servico_desejado']."
                        </li>";
                  ...
                  <ul style="text-align: left;" class="list-group">
                    ...
                    $sql_servico = "SELECT * FROM `servico` WHERE  `cod_professor` = $cod_professor && `status`=2";
                    $query_servico = mysqli_query($conn, $sql_servico);
                    $numero_servico = mysqli_num_rows($query_servico); 
                    if ($numero_servico !=0) {
                      while ($vetor_servico = mysqli_fetch_array($query_servico)) {
                        ...
                        $vetor_veiculo = mysqli_fetch_array($veiculo_resultado);
                        echo "<li class='list-group-item itens'><p style='display:block;'>
                        Veículo:".$vetor_veiculo['placa']."<a href='chat_professor.php?cod_servico=".$vetor_servico['cod_servico']."' style='float:right; right:0px;'><i class='fas fa-external-link-alt'></i></a>
                  ...
                  <ul style="text-align: left;" class="list-group">
                    ...
                    $sql_servico = "SELECT * FROM `servico` WHERE  `cod_professor` = $cod_professor && `status`=3";
                    $query_servico = mysqli_query($conn, $sql_servico);
                    $numero_servico = mysqli_num_rows($query_servico); 
                    if ($numero_servico !=0) {
                      while ($vetor_servico = mysqli_fetch_array($query_servico)) {
                        ...
                        $vetor_veiculo = mysqli_fetch_array($veiculo_resultado);
                        echo "<li class='list-group-item itens'><p style='display:block;'>
                        Veículo:".$vetor_veiculo['placa']."<a href='chat_professor.php?cod_servico=".$vetor_servico['cod_servico']."'style='float:right; right:0px;margin-left:5px;'><i class='fas fa-external-link-alt'></i></a> <a href='#' style='float:right;' data-toggle='modal' data-target='#exampleModal'><i class='fas fa-share-square'></i></a>
                        Protocolo:   ".$vetor_servico['protocolo']."<br>Status: Aguardando carro ser entregue<br>
                  ...
                  <ul style="text-align: left;" class="list-group">
                    ...
                    $sql_servico = "SELECT * FROM `servico` WHERE  `cod_professor` = $cod_professor && `status`=4";
                    $query_servico = mysqli_query($conn, $sql_servico);
                    $numero_servico = mysqli_num_rows($query_servico); 
                    if ($numero_servico !=0) {
                      while ($vetor_servico = mysqli_fetch_array($query_servico)) {
      ...
      <script>
        ...
        $(".codigof").click(function() {
          ...
          $(location).attr('href', 'perfil_professor.php');
